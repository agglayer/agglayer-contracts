...
async function main() {
    ...
    const mandatoryParameters = ['type', 'aggLayerGatewayAddress', 'defaultAggchainSelector'];
    ...
    const { type, aggLayerGatewayAddress, defaultAggchainSelector } = params;
    ...
    if (type === transactionTypes.TIMELOCK) {
        ...
        try {
            const tx = await aggLayerGateway.unsetDefaultAggchainVKey(defaultAggchainSelector);
            await tx.wait();
            outputJson.aggLayerGatewayAddress = aggLayerGatewayAddress;
            outputJson.defaultAggchainSelector = defaultAggchainSelector;
