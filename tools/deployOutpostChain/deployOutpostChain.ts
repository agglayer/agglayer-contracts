...
async function main() {
    ...
    // Step 4: Deploy AgglayerManagerGERL2 with pre-calculated Bridge address
...
async function deployBridgeL2SovereignChain(
     ...
     *        IAgglayerBridgeV2(msg.sender).getWrappedTokenBridgeImplementation(), // ← CALLBACK
     *        new bytes(0)
     *    ) {
     *        _changeAdmin(IAgglayerBridgeV2(msg.sender).getProxiedTokensManager()); // ← CALLBACK
     *    }
     *
...
/**
 * Deploy AgglayerManagerGERL2 with proxy pattern using prepareUpgrade and centralized ProxyAdmin
 */
async function deployGlobalExitRootManagerL2SovereignChain(
    ...
    const GERManagerFactory = await ethers.getContractFactory('AgglayerManagerGERL2', deployer);
    ...
    // Wait for 5 confirmations for correct etherscan verification
    await deployTx?.wait(5);
    logger.info(`✅ AgglayerManagerGERL2 implementation deployed: ${gerImplementation.target}`);
    ...
    logger.info(`✅ AgglayerManagerGERL2 implementation: ${gerImplementation.target}`);
    logger.info(`✅ AgglayerManagerGERL2 proxy (initialized): ${gerProxy.target}`);
