...
contract PolygonZkEVMGlobalExitRootV2Test is Test, TestHelpers, ZkEVMCommon, PolygonZkEVMGlobalExitRootV2Deployer {
    ...
    function testRevert_updateExitRoot_onlyAllowedContracts() public {
        ...
        vm.expectRevert(IBasePolygonZkEVMGlobalExitRoot.OnlyAllowedContracts.selector);
    ...
    function test_updateExitRoot_asBridge() public {
        ...
        assertEq(polygonZkEVMGlobalExitRootV2.lastMainnetExitRoot(), newRootBridge);
        assertEq(polygonZkEVMGlobalExitRootV2.lastRollupExitRoot(), bytes32(0));

        bytes32 expectedGlobalExitRoot = calculateGlobalExitRoot(newRootBridge, bytes32(0));
        assertEq(polygonZkEVMGlobalExitRootV2.getLastGlobalExitRoot(), expectedGlobalExitRoot);
    }
    ...
    function test_updateExitRoot_asRollupManager() public {
        ...
        assertEq(polygonZkEVMGlobalExitRootV2.lastMainnetExitRoot(), bytes32(0));
        assertEq(polygonZkEVMGlobalExitRootV2.lastRollupExitRoot(), newRootRollup);

        bytes32 expectedGlobalExitRoot = calculateGlobalExitRoot(bytes32(0), newRootRollup);
        assertEq(polygonZkEVMGlobalExitRootV2.getLastGlobalExitRoot(), expectedGlobalExitRoot);
    }
