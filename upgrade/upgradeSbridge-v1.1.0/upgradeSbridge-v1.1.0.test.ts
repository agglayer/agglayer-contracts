...
describe('Should shadow fork network, execute upgrade and validate Upgrade', () => {
    it('Should shadow fork network, execute upgrade and validate Upgrade', async () => {
        ...
        const proxyAdmin = proxyAdminFactory.attach(proxyAdminAddress) as any;
        ...
        if (!ethers.isAddress(proposerRoleAddress)) {
        ...
        // Get contract - using the current version since we're upgrading to v1.1.0
        ...
        // Bridge prev params - assuming we're upgrading from v1.0.0
        const version = await sovereignBridgeContract.BRIDGE_SOVEREIGN_VERSION();
        expect(version).to.equal('v10.1.2');
        ...
        expect(await sovereignBridgeContract.bridgeLib()).to.equal(
            upgradeOutput.deployedContracts.bridgeLib,
        );

        // Check can not be reinitialized - using the no-parameter initialize function
        await expect(
            sovereignBridgeContract['initialize()'](),
        ).to.be.revertedWithCustomError(sovereignBridgeContract, 'InvalidInitializeFunction');
        logger.info(`✓ Checked bridge contract storage parameters`);

        logger.info('Finished shadow fork upgrade test successfully!');
    }).timeout(0);
});
